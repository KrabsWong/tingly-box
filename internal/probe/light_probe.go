...
func (l *LightProber) runChatEndpoint(ctx context.Context, provider *typ.Provider,
	...
	l.runOpenAIEndpoint(ctx, provider, success, msg, rt, "Chat endpoint accessible",
		func(c client.OpenAIClientInterface, pctx context.Context) (*Result, error) {
			return probeOpenAIChat(pctx, c, "gpt-3.5-turbo", "Hi", E2EModeSimple, ThinkingNone)
		})
}
...
func (l *LightProber) runResponsesEndpoint(ctx context.Context, provider *typ.Provider,
	...
	l.runOpenAIEndpoint(ctx, provider, success, msg, rt, "Responses API endpoint accessible",
		func(c client.OpenAIClientInterface, pctx context.Context) (*Result, error) {
			return probeOpenAIResponses(pctx, c, "gpt-4o", "Hi", E2EModeSimple, ThinkingNone)
		})
}
...
func (l *LightProber) runModelsEndpoint(ctx context.Context, provider *typ.Provider,
	...
	case protocol.APIStyleOpenAI:
		c := l.pool.GetOpenAIClient(ctx, provider, "")
		if c == nil {
			report(false, "Failed to create OpenAI client", 0, "")
			return
		}
		lister = c
	case protocol.APIStyleAnthropic:
		...
		if c == nil {
			report(false, "Failed to create Anthropic client", 0, "")
			return
		}
		lister = c
	case protocol.APIStyleGoogle:
		...
		if c == nil {
			report(false, "Failed to create Google client", 0, "")
			return
		}
		lister = c
	default:
	...
	default:
		...
		report(true, fmt.Sprintf("Models endpoint accessible - %d models found", modelCount), modelCount, "")
	}
}
