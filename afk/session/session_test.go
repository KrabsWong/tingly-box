...
// A compaction hides the messages it replaced and carries its summary into the
// first survivor. Reloading must produce the same conversation the run was
// using when it compacted, or the next turn resumes from a different history
// than the model was actually shown.
func TestCompaction_ProjectionHidesReplacedMessages(t *testing.T) {
	path := filepath.Join(t.TempDir(), "s.jsonl")
	s, err := Open(path)
	require.NoError(t, err)
	...
	require.NoError(t, s.AppendCompaction("[summary] we discussed q1 and q2", 4))

	reopened, err := Open(path)
	require.NoError(t, err)
	msgs := reopened.Messages()

	require.Len(t, msgs, 2, "the four replaced messages should be hidden")
	assert.Equal(t, anthropic.BetaMessageParamRoleUser, msgs[0].Role)
	require.Len(t, msgs[0].Content, 2, "summary rides on the first survivor")
	assert.Equal(t, "[summary] we discussed q1 and q2", msgs[0].Content[0].OfText.Text)
	assert.Equal(t, "q3", msgs[0].Content[1].OfText.Text)
	assert.Equal(t, "a3", msgs[1].Content[0].OfText.Text)
}

// The replaced messages stay in the file: the log is append-only, and keeping
// them means a compaction is auditable after the fact.
func TestCompaction_ReplacedMessagesStayInTheFile(t *testing.T) {
	path := filepath.Join(t.TempDir(), "s.jsonl")
	s, err := Open(path)
	require.NoError(t, err)
	require.NoError(t, s.Append(userMsg("secret-q"), assistantMsg("secret-a"), userMsg("kept")))
	require.NoError(t, s.AppendCompaction("[summary]", 2))

	data, err := os.ReadFile(path)
	require.NoError(t, err)
	assert.Contains(t, string(data), "secret-q", "compaction must not rewrite history on disk")
	assert.Equal(t, 4, strings.Count(strings.TrimRight(string(data), "\n"), "\n")+1,
		"three messages plus the compaction entry")
}

// Compacting twice keeps only the newest summary; the older one is already
// folded into the text the newer was written from.
func TestCompaction_SecondCompactionSupersedesTheFirst(t *testing.T) {
	path := filepath.Join(t.TempDir(), "s.jsonl")
	s, err := Open(path)
	require.NoError(t, err)

	require.NoError(t, s.Append(userMsg("q1"), assistantMsg("a1"), userMsg("q2"), assistantMsg("a2")))
	require.NoError(t, s.AppendCompaction("[first summary]", 2))
	require.NoError(t, s.Append(userMsg("q3"), assistantMsg("a3")))
	require.NoError(t, s.AppendCompaction("[second summary]", 2))
	...
	assert.Equal(t, "[second summary]", msgs[0].Content[0].OfText.Text)
	assert.Equal(t, "q3", msgs[0].Content[1].OfText.Text)
	for _, m := range msgs {
		for _, b := range m.Content {
			if b.OfText != nil {
				assert.NotEqual(t, "[first summary]", b.OfText.Text, "only the newest summary applies")
			}
		}
	}
}

// AppendNew has to reason about the projected conversation, not the raw entry
// count, or the turn after a compaction would re-append everything it kept.
func TestCompaction_AppendNewStaysCorrectAfterCompaction(t *testing.T) {
	path := filepath.Join(t.TempDir(), "s.jsonl")
	s, err := Open(path)
	require.NoError(t, err)
	...
	full := s.Messages() // what the run is holding after compaction
	require.Len(t, full, 2)

	added, err := s.AppendNew(append(full, userMsg("q3")))
	require.NoError(t, err)
	assert.Equal(t, 1, added, "only the genuinely new message should be written")
