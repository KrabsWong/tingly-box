...
export interface UseChatProbeResult {
    ...
    run: (botUUID: string, chatID: string, targetID: string, capability: ChatCapability) => Promise<void>;
...
export function useChatProbe(): UseChatProbeResult {
    ...
    const runNotify = useCallback(async (botUUID: string, targetID: string, started: number): Promise<ChatProbeResult> => {
        const result = await api.notifyBot(botUUID, {
            target: {kind: 'direct_chat', id: targetID},
    ...
    const runConfirm = useCallback(async (botUUID: string, targetID: string, started: number): Promise<ChatProbeResult> => {
    ...
    const run = useCallback(async (botUUID: string, chatID: string, targetID: string, capability: ChatCapability) => {
        const k = key(chatID, capability);
        setRun(k, true);
        const started = Date.now();
        try {
            const r = capability === 'notify'
                ? await runNotify(botUUID, targetID, started)
                : await runConfirm(botUUID, targetID, started);
            setResult(k, r);
        } finally {
            setRun(k, false);
        }
